...
def fetch_robust_data(ticker):
    try:
        ...
        # Yahoo MultiIndex vs Single Index kezelés + NaN dobás
...
if rows:
    ...
    def style_diff(v):
        return f'color: {"#27ae60" if v > 0 else "#e74c3c" if v < 0 else "white"}; font-weight: bold;'

    st.table(disp.style.map(style_diff, subset=["Day change %"]).format("{:+.2f}%", subset=["Day change %"]))

    st.markdown("---")
    st.subheader("Portfólió megoszlása")
    fig = px.pie(df, values='Value USD', names='Név', color_discrete_sequence=px.colors.qualitative.Pastel)
    fig.update_traces(textinfo='percent+label', textposition='inside')
    fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
    st.plotly_chart(fig, use_container_width=True)
else:
    st.info("Nincs adat. Várj egy kicsit vagy frissíts!")
